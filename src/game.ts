...
export default class Game implements IGame {
	...
	private onMouseUp = (event: MouseEvent): void => {
		...
		const stack = this.stacks.find(
			(currentStack: IStack) => event.x >= currentStack.x - 28 && event.x <= currentStack.x + 28 && event.y >= currentStack.y - 53,
		);
		const stackIndex = stack ? this.stacks.indexOf(stack) : this.hand.lastStackIndex;
