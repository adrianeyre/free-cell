...
import type IStackProps from './interfaces/stack-props';
...
export default class Stack implements IStack {
	...
	public isValidMove = (currentCards: ICard[], nextCard: ICard = this.cards[this.cards.length - 1]): boolean => {
		...
		if (this.isHomeSquare) {
			return (
				(this.isStackEmpty() && this.isCardAce(currentCard)) ||
		...
		return (
			(this.isStackEmpty() && isAllowedToAddMoreThanOne) ||
			(this.isCardsInStack() &&
				isAllowedToAddMoreThanOne &&
				this.isSuiteDifferentColour(currentCard, nextCard) &&
				this.isCostOneLess(currentCard, nextCard))
		);
	};
